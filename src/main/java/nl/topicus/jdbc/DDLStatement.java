...
class DDLStatement {
  ...
  enum ObjectType {
    ...
    abstract boolean exists(CloudSpannerConnection connection, String objectName)
        throws SQLException;
  }
  ...
  private DDLStatement(Command command, ObjectType objectType, ExistsStatement existsStatement,
      String objectName, String sql) {
    ...
    for (String sql : sqlList) {
      ...
      if (tokens.size() >= 3) {
        ...
        boolean nullFilteredIndex = tokens.get(1).equalsIgnoreCase(NULL_FILTERED_KEYWORD)
            && tokens.get(2).equalsIgnoreCase(INDEX_KEYWORD);
        boolean uniqueNullFilteredIndex =
            tokens.size() >= 4 && ((tokens.get(1).equalsIgnoreCase(UNIQUE_KEYWORD)
