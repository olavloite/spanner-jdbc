...
class ValueBinderExpressionVisitorAdapter<R> extends AbstractSpannerExpressionVisitorAdapter {
  private static final Pattern SPLIT_ON_COMMA_PATTERN =
      Pattern.compile(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)");
  ...
  private R splitAndSetArrayValue(String arrayString) {
    ...
    String[] array =
        SPLIT_ON_COMMA_PATTERN.split(arrayString.substring(1, arrayString.length() - 1));
      ...
      if (val != null) {
        ...
        try {
          ...
          throw new IllegalArgumentException(String.format(
              "Invalid date value: '%s'. Date values must be specified in the format yyyy-MM-dd.",
              date), e);
        }
      }
      index++;
  ...
  private Timestamp[] convertTimestampArray(String[] array) {
    ...
    for (String val : array) {
      if (val != null) {
        StringBuilder date =
            new StringBuilder(val.substring(5, val.length() - 2).replace(' ', 'T'));
  ...
  private R setArrayValue(Object value) {
    if (boolean[].class.isAssignableFrom(value.getClass())) {
      ...
      return binder.toTimestampArray(
          CloudSpannerConversionUtil.toCloudSpannerTimestamps((Timestamp[]) value));
    } else if (String[].class.isAssignableFrom(value.getClass())) {
