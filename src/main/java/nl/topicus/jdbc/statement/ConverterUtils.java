...
public class ConverterUtils {
  ...
  public static int calculateActualBatchSize(int numberOfCols, Connection destination,
  ...
  public static int getNumberOfIndices(Connection destination, String catalog, String schema,
      String table) throws SQLException {
    int count = 0;
    try (ResultSet indices =
        destination.getMetaData().getIndexInfo(catalog, schema, table, false, false)) {
      while (indices.next())
        count++;
    }
    return count;
  ...
  public static List<String> getQuotedColumnNames(Connection destination, String catalog,
  ...
  /**
   * @param connection The connection to use to estimate the row size
   * @param catalog The catalog of the table
   * @param schemaPattern The schema of the table
   * @param tableNamePattern The name of the table
   * @param columnNamePattern The name of the column(s) to check
   * @return The estimated size in bytes of one row of the specified columns of the specified table
   * @throws SQLException If a database error occurs while getting the table meta data
   */
  public static int getEstimatedRowSizeInCloudSpanner(Connection connection, String catalog,
