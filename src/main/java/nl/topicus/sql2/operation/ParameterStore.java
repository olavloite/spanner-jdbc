...
public class ParameterStore
{
	...
	private SqlType[] types = new SqlType[10];
	...
	SqlType getType(int parameterIndex)
	{
		int arrayIndex = parameterIndex - 1;
	...
	void setType(int parameterIndex, SqlType type)
	{
	...
	{
		setParameter(parameterIndex, value, null, null, null);
	}

	void setParameter(int parameterIndex, Object value, SqlType sqlType)
	{
		setParameter(parameterIndex, value, sqlType, null, null);
	}

	void setParameter(int parameterIndex, Object value, SqlType sqlType, Integer scaleOrLength, String column)
	{
			...
			try (ResultSet rsCols = connection.getMetaData().getColumns(null, null, table, null))
			{
				while (rsCols.next())
				{
					...
					if (arrayIndex > -1)
					{
						...
						types[arrayIndex] = getSqlType(rsCols.getInt("DATA_TYPE"));
	...
	private SqlType getSqlType(Integer type)
	{
		// TODO: Implement
		return null;
	}
