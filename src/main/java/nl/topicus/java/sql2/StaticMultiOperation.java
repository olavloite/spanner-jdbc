...
public interface StaticMultiOperation<T> extends OutOperation<T> {
  ...
  // Covariant overrides
  @Override
  public StaticMultiOperation<T> resultProcessor(Function<Result.OutParameterMap, ? extends T> processor);
  ...
  @Override
  public StaticMultiOperation<T> set(String id, CompletableFuture<?> valueFuture, SqlType type);

  @Override
  public StaticMultiOperation<T> set(String id, CompletableFuture<?> valueFuture);

  @Override
  public default StaticMultiOperation<T> set(String id, CompletionStage value) {
    return set(id, value.toCompletableFuture());
  }

  @Override
  public default StaticMultiOperation<T> set(String id, CompletionStage value, SqlType type) {
    return set(id, value.toCompletableFuture(), type);
  }

  @Override
  public StaticMultiOperation<T> timeout(long milliseconds);
