...
public interface SqlRef<T> {
   ...
   */
  public default CompletableFuture<T> fetch() {
    return fetchOperation().submit().toCompletableFuture();
  }
   ...
   * @return a Future that will complete when the submitted Operation completes.
   */
  public default CompletableFuture<Void> store(T value) {
    return storeOperation(value).submit().toCompletableFuture();
  }
}
