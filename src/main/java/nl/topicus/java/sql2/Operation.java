...
public interface Operation<T> {
   ...
   * automatically. {@link Long#MAX_VALUE} is effectively forever. The time is
   * counted from the beginning of Operation execution. The Operation will not
   * be canceled before {@code milliseconds} after the beginning of execution.
   * Some time at least {@code milliseconds} after the beginning of execution,
   ...
   *
   * @param milliseconds minimum number of milliseconds before attempting to
   * cancel
  ...
  /**
   * Submit this {@link Operation} and block waiting for the {@link Operation}
   * to complete. When it completes return the result.
   *
   * @return the result of this {@link Operation}
   * @throws InterruptedException if the thread executing the submitted {@link Operation} is interrupted
   * @throws ExecutionException if the submitted {@link Operation} completes exceptionally
   */
  public default T submitAndWait() throws InterruptedException, ExecutionException {
