...
public class CloudSpannerXAConnectionTest {
  ...
  public void testExpectedErrors() throws SQLException {
    CloudSpannerXAConnection subject = createSubject();
    testExpectedError(() -> subject.getConnection().prepareCall("TEST"),
        SQLFeatureNotSupportedException.class);
    testExpectedError(() -> subject.start(getRandomXid(), XAResource.TMSUCCESS),
        CloudSpannerXAException.class, CloudSpannerXAException.INVALID_FLAGS);
    testExpectedError(() -> subject.start(null, XAResource.TMNOFLAGS),
        CloudSpannerXAException.class, CloudSpannerXAException.XID_NOT_NULL);
    testExpectedError(() -> subject.start(getRandomXid(), XAResource.TMRESUME),
        CloudSpannerXAException.class, CloudSpannerXAException.SUSPEND_NOT_IMPLEMENTED);

    testExpectedError(() -> subject.end(getRandomXid(), XAResource.TMNOFLAGS),
        CloudSpannerXAException.class, CloudSpannerXAException.INVALID_FLAGS);
  ...
  private <T extends Throwable> void testExpectedError(RunnableWithException runnable,
      Class<T> expectedException) {
    testExpectedError(runnable, expectedException, null);
  }

  private <T extends Throwable> void testExpectedError(RunnableWithException runnable,
      Class<T> expectedException, String message) {
    try {
      runnable.run();
    } catch (Exception e) {
      Assert.assertEquals(
          "Unexpected error: " + e.getMessage() + " - Expected " + expectedException.getName(),
