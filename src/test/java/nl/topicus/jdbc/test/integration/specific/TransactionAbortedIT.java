...
public class TransactionAbortedIT extends AbstractSpecificIntegrationTest {
  ...
  private void runTransactionTests() throws SQLException {
    ...
    getConnection().createStatement()
        .executeUpdate("insert into test (id, name) values (2, 'two')");
    ...
    getConnection().createStatement()
        .executeUpdate("insert into test (id, name) values (3, 'three')");
    try (ResultSet rs =
        getConnection().createStatement().executeQuery("FORCE_TRANSACTION_ABORT_ON_COMMIT")) {
      while (rs.next()) {
    ...
    getConnection().createStatement()
        .executeUpdate("insert into test (id, name) values (4, 'four')");
