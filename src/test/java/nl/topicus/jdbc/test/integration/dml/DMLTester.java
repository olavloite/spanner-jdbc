...
public class DMLTester {
  ...
  private void runSelectTests() throws SQLException {
    String sql =
        "SELECT * FROM TESTCHILD@{FORCE_INDEX=IDX_TESTCHILD_DESCRIPTION} WHERE DESCRIPTION LIKE ?";
  ...
  private void runBulkInsertTests() throws SQLException {
    ...
    for (int i = 0; i < BULK_INSERT_COUNT; i++) {
      ...
      String sql =
          "INSERT INTO TEST SELECT ID + (SELECT MAX(ID) FROM TEST), UUID, ACTIVE, AMOUNT, DESCRIPTION, CREATED_DATE, LAST_UPDATED FROM TEST";
      PreparedStatement statement = connection.prepareStatement(sql);
      statement.executeUpdate();
      connection.commit();
      verifyTableContents("SELECT COUNT(*) FROM TEST",
          Double.valueOf(Math.pow(2, (i + 2))).longValue());
      verifyTableContents("SELECT MAX(ID) FROM TEST",
          Double.valueOf(Math.pow(2, (i + 2))).longValue());
  ...
  private void runBulkInsertWithExceptionTest() throws SQLException {
    ...
    String sql =
        "INSERT INTO TEST SELECT ID, UUID, ACTIVE, AMOUNT, DESCRIPTION, CREATED_DATE, LAST_UPDATED FROM TEST";
    PreparedStatement statement = connection.prepareStatement(sql);
    try {
      statement.executeUpdate();
      connection.commit();
  ...
  private void verifyTableContentsAfterInsert() throws SQLException {
    ...
    verifyTableContents("SELECT LAST_UPDATED FROM TEST WHERE ID=1",
        new Timestamp(2017 - 1900, 2, 18, 7, 0, 0, 0));
    ...
    verifyTableContents("SELECT DESCRIPTION FROM TESTCHILD WHERE ID=1 AND CHILDID=1",
        "Child description 1.1");
  ...
  private void runUpdateTests(ExecuteMode mode)
  ...
  private void verifyTableContentsAfterUpdate() throws SQLException {
    verifyTableContents("SELECT UUID FROM TEST WHERE ID=1",
        DatatypeConverter.parseHexBinary("aabbcc"));
    ...
    verifyTableContents("SELECT LAST_UPDATED FROM TEST WHERE ID=2",
        new Timestamp(2017 - 1900, 1, 17, 8, 0, 0, 0));
  }
    ...
    try (ResultSet rs = connection.createStatement().executeQuery(sql)) {
      if (rs.next()) {
        ...
        if (expectedValue != null && expectedValue instanceof byte[]) {
          ...
          throw new SQLException("Expected value: " + String.valueOf(expectedValue)
              + ", found value: " + String.valueOf(value));
        }
      } else {
