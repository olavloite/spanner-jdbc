...
public class ResultSetMetaDataStringMaxLengthIT extends AbstractSpecificIntegrationTest
{
	...
	{
		try (ResultSet rs = getConnection().createStatement().executeQuery("select * from test_table"))
		{
	...
	{
		try (ResultSet rs = getConnection().createStatement().executeQuery("select count(*) from test_table"))
		{
			if (rs.next())
				return rs.getLong(1);
		}
