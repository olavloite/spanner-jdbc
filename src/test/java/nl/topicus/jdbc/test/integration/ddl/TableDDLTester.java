...
/**
 * Class for testing Table DDL statements, such as CREATE TABLE, ALTER TABLE, DROP TABLE.
 * 
 * @author loite
 *
 */
public class TableDDLTester {
    ...
    try (ResultSet rs = connection.getMetaData().getPrimaryKeys("", "", table)) {
      ...
      if (!expectedColumns.equalsIgnoreCase(colsFound))
        throw new AssertionError(
            "Primary key columns expected: " + expectedColumns + ", found: " + colsFound);
    }
  }
  ...
  private void verifyColumn(String table, String column, boolean mustExist, String type)
      throws SQLException {
    String colFound = "";
    String typeFound = "";
    ...
    if (mustExist) {
      ...
      if (!type.equalsIgnoreCase(typeFound))
        throw new AssertionError(
            "Column " + column + " has type " + typeFound + ", expected was " + type);
    } else {
      if (count != 0)
  ...
  private void executeDdl(String file, boolean expectsError)
      throws IOException, URISyntaxException, SQLException {
