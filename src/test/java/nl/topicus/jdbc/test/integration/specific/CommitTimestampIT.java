...
/**
 * Integration tests for tables with a timestamp column that may contain the commit timestamp
 * column.
 * 
 * @author loite
 *
...
public class CommitTimestampIT extends AbstractSpecificIntegrationTest {
  ...
  public void test1_CreateTable() throws SQLException {
    ...
    // Verify the columns
    try (ResultSet rsCols =
        getConnection().getMetaData().getColumns("", "", "TEST_WITH_COMMIT_TS", null)) {
  ...
  public void test2_InsertDataWithValueForTimestamp() throws SQLException {
    ...
    try (ResultSet rs =
        getConnection().createStatement().executeQuery("SELECT * FROM TEST_WITH_COMMIT_TS")) {
      assertTrue(rs.next());
