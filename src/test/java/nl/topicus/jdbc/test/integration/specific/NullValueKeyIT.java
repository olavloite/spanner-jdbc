...
public class NullValueKeyIT extends AbstractSpecificIntegrationTest
{
	...
	{
		String sql = "create table table_with_null_keys (id1 int64 not null, id2 int64, name string(100)) primary key (id1, id2)";
		getConnection().createStatement().executeUpdate(sql);
		try (ResultSet rs = getConnection().getMetaData().getColumns(null, null, "table_with_null_keys", null))
		{
			while (rs.next())
			{
	...
	{
		try (ResultSet rs = getConnection().createStatement().executeQuery("select count(*) from table_with_null_keys"))
		{
			if (rs.next())
				return rs.getLong(1);
		}
