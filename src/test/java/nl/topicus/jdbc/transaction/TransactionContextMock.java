...
class TransactionContextMock {
  ...
  TransactionContext createTransactionContextMock() {
    ...
    when(res.executeQuery(XATransaction
        .getPreparedMutationsStatement(RecoveredXid.xidToString(XID_WITHOUT_MUTATIONS))))
  ...
  private List<String> createSerializedMutations() throws SQLException {
    ...
    res.add(
        XATransaction.serializeMutation(Mutation.newInsertBuilder("FOO").set("ID").to(1L).build()));
    res.add(
        XATransaction.serializeMutation(Mutation.newInsertBuilder("FOO").set("ID").to(2L).build()));
    res.add(
        XATransaction.serializeMutation(Mutation.newInsertBuilder("FOO").set("ID").to(3L).build()));
