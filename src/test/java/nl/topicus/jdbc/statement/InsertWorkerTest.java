...
public class InsertWorkerTest
{
	...
	public static class UpdateTests
	{
		...
		public void testUpdateStatementWithoutWhereClause() throws SQLException
		{
			...
			InsertWorker worker = (InsertWorker) mutations.getWorker();
			Assert.assertEquals("SELECT `ID`, 1, 2 FROM `FOO`", worker.select.toString());
			Assert.assertTrue(worker.isOnDuplicateKeyUpdate());
			Assert.assertEquals(
					"INSERT INTO `FOO` (`ID`, `COL1`, `COL2`) SELECT `ID`, 1, 2 FROM `FOO` ON DUPLICATE KEY UPDATE FOO = BAR",
		...
		public void testUpdateStatementWithWhereClause() throws SQLException
		{
			...
			InsertWorker worker = (InsertWorker) mutations.getWorker();
			Assert.assertEquals("SELECT `ID`, COL1 + COL2, COL2 * 1.1 FROM `FOO` WHERE COL1 < 100",
					worker.select.toString());
			Assert.assertTrue(worker.isOnDuplicateKeyUpdate());
			Assert.assertEquals(
					"INSERT INTO `FOO` (`ID`, `COL1`, `COL2`) SELECT `ID`, COL1 + COL2, COL2 * 1.1 FROM `FOO` WHERE COL1 < 100 ON DUPLICATE KEY UPDATE FOO = BAR",
					worker.getInsert().toString());
