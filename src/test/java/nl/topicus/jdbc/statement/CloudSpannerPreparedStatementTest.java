...
public class CloudSpannerPreparedStatementTest {
  ...
  public static class DeleteStatementTests {
    ...
    public void testDeleteStatementWithoutIdInWhere() throws SQLException {
      ...
      Assert.assertEquals(DeleteWorker.class, mutations.getWorker().getClass());
      Assert.assertEquals("SELECT `FOO`.`ID` FROM `FOO` WHERE BAR = 1",
          mutations.getWorker().select.toString());
    }
    ...
    public void testDeleteStatementWithLargerThan() throws SQLException {
      ...
      Assert.assertEquals(DeleteWorker.class, mutations.getWorker().getClass());
      Assert.assertEquals("SELECT `FOO`.`ID` FROM `FOO` WHERE ID > 2",
          mutations.getWorker().select.toString());
    }
    ...
    public void testDeleteStatementWithNotEquals() throws SQLException {
      ...
      Assert.assertEquals(DeleteWorker.class, mutations.getWorker().getClass());
      Assert.assertEquals("SELECT `FOO`.`ID` FROM `FOO` WHERE ID != 1",
          mutations.getWorker().select.toString());
    }
    ...
    public void testDeleteStatementWithExists() throws SQLException {
      Mutations mutations =
          getMutations("DELETE FROM FOO WHERE EXISTS (SELECT ID FROM FOO WHERE ID=1)");
      Assert.assertEquals(DeleteWorker.class, mutations.getWorker().getClass());
      Assert.assertEquals(
          "SELECT `FOO`.`ID` FROM `FOO` WHERE EXISTS (SELECT ID FROM FOO WHERE ID = 1)",
          mutations.getWorker().select.toString());
    }
    ...
    public void testDeleteStatementWithAll() throws SQLException {
      Mutations mutations =
          getMutations("DELETE FROM FOO WHERE ID=ALL (SELECT ID FROM FOO WHERE ID=1)");
      Assert.assertEquals(DeleteWorker.class, mutations.getWorker().getClass());
      Assert.assertEquals(
          "SELECT `FOO`.`ID` FROM `FOO` WHERE ID = ALL (SELECT ID FROM FOO WHERE ID = 1)",
          mutations.getWorker().select.toString());
    }
    ...
    public void testDeleteStatementWithAny() throws SQLException {
      Mutations mutations =
          getMutations("DELETE FROM FOO WHERE ID=ANY (SELECT ID FROM FOO WHERE ID=1)");
      Assert.assertEquals(DeleteWorker.class, mutations.getWorker().getClass());
      Assert.assertEquals(
          "SELECT `FOO`.`ID` FROM `FOO` WHERE ID = ANY (SELECT ID FROM FOO WHERE ID = 1)",
          mutations.getWorker().select.toString());
    }
  ...
  public static class UpdateStatementTests {
    ...
    public void testUpdateStatementWithMultipleWhereClauses() throws SQLException {
      ...
      Assert.assertArrayEquals(new String[] {"1", "2", "1", "1"},
    ...
    private CloudSpannerPreparedStatement prepareUnsupportedStatementTest(String sql)
        throws SQLException {
      thrown.expect(CloudSpannerSQLException.class);
      thrown.expectMessage("may not be called on a PreparedStatement");
      CloudSpannerConnection connection =
          Mockito.mock(CloudSpannerConnection.class, CALLS_REAL_METHODS);
  ...
  public static class InsertStatementTests {
    ...
    public void testInsertStatementWithoutColumns() throws SQLException {
      ...
      assertSingleInsert(getMutation("INSERT INTO FOO VALUES (1, 'two', 0xaa)"),
          Mutation.Op.INSERT);
    }
  ...
  public static class BatchTests {
    ...
    @Test
    public void testBatchedInsertStatements() throws SQLException, NoSuchFieldException,
        SecurityException, IllegalArgumentException, IllegalAccessException {
      Field batchMutationsField =
          CloudSpannerPreparedStatement.class.getDeclaredField("batchMutations");
  ...
  public static class DDLStatementTests {
    ...
    /**
     * TODO: quote the identifier in primary key clause when this is fixed in the underlying SQL
     * Parser
     * 
     * @throws SQLException
     */
  ...
  public static class ParameterTests {
    ...
    public void testParameters() throws SQLException, MalformedURLException {
      ...
      sql.append(IntStream.range(0, numberOfParams).mapToObj(i -> "COL" + i)
          .collect(Collectors.joining(", ")));
      sql.append(") VALUES (");
      sql.append(
          IntStream.range(0, numberOfParams).mapToObj(i -> "?").collect(Collectors.joining(", ")));
      sql.append(")");

      CloudSpannerPreparedStatement ps =
          CloudSpannerTestObjects.createPreparedStatement(sql.toString());
  ...
  public static class SelectStatementTests {
    ...
    public void testExecuteQueryWithDML() throws SQLException, MalformedURLException {
      thrown.expect(SQLException.class);
      thrown
          .expectMessage("SQL statement not suitable for executeQuery. Expected SELECT-statement.");
    ...
    public void testSelectWithInterval() throws SQLException {
      String sql =
          "SELECT n.news_type_id, n.text FROM news2 AS n INNER JOIN news_types nt ON (nt.news_type_id = n.news_type_id) "
    ...
    public void testSelectWithParameterInSelectItem() throws SQLException, MalformedURLException {
      String sql =
          "SELECT CASE WHEN (n.accountId > ?) THEN true ELSE false END FROM Notification n";
      CloudSpannerPreparedStatement ps = CloudSpannerTestObjects.createPreparedStatement(sql);
      ps.setLong(1, 1000L);
      try (ResultSet rs = ps.executeQuery()) {
      }
  ...
  private static boolean isDDLStatement(String sql) throws SQLException {
    boolean res = false;
    CloudSpannerPreparedStatement ps = new CloudSpannerPreparedStatement(sql,
        mock(CloudSpannerConnection.class), mock(DatabaseClient.class));
    try {
      Method isDDLStatement =
          CloudSpannerPreparedStatement.class.getDeclaredMethod("isDDLStatement");
